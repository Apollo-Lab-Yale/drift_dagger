...
class RecedingHorizonVisuomotorDataset(BaseDataset):
    def __init__(self,
                 dataset_path: str,
                 state_dim: int=9,
                 action_dim: int=9,
    ...
    def __len__(self):
        if self.total_num_expert_labels == 0:
            print("DO NOT SAMPLE WHEN THERE ARE NO EXPERT LABELS")
            return 1
    ...
    def __getitem__(self, idx):
        ...
        return sampled_rgb_images, sampled_system_states, sampled_actions

    def create_new_rollout_dataset(self):
        super().create_new_rollout_dataset()
        rgb_images_ds = self.curr_rollout_ds.create_dataset('rgb_images',
                                                           shape=(0, self.img_height, self.img_width, 3),
                                                           chunks=(1, self.img_height, self.img_width, 3),
                                                           dtype=np.uint8)

    def append_to_curr_rollout_dataset(self, rgb_img, state, action, is_expert_label=True):
        if self.curr_rollout_ds is None:
            raise Exception("No current rollout dataset exists! Please create one first.")
        self.curr_rollout_ds["rgb_images"].append(rgb_img[None, ...])
    ...
    def remove_curr_rollout_dataset(self):
        zarr_rollout_dataset_path = f"{self.dataset_path}/{self.curr_rollout_idx}"

        if Path(zarr_rollout_dataset_path).exists():
            print(f"Removing existing zarr dataset at {zarr_rollout_dataset_path}")
            shutil.rmtree(zarr_rollout_dataset_path)